 ...
 */
type Linked = { text: string; hyperlink: string };
type Cell = string | number | boolean | Date | null | Linked;

/** Every reader normalises to this, so there is one row-reading path, not three. */
type Sheet = { name: string; rows: Cell[][] };

const isLinked = (value: Cell): value is Linked =>
  typeof value === "object" && value !== null && !(value instanceof Date) && "hyperlink" in value;

const isEmptyRow = (row: Cell[]) => row.every((cell) => cell === null || cell === undefined || cell === "");

function fromWorkbook(workbook: ExcelJS.Workbook): Sheet[] {
