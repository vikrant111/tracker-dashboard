...
| **Two vertical seams either side of the header** | the topbar's blurred backdrop was `absolute inset-0`, and the bar lives inside a `max-w-[1400px]` container — so on any wider screen it stopped at the container edge | it bleeds a **fixed** distance past each gutter (`-left-24`, `sm:-left-40`). No edge at any width, and nothing wider than the screen |
| **The whole page scrolled sideways on a phone** | the fix above was first written as `-left-[50vw] -right-[50vw]` — 200vw of element, held back only by `overflow-x: hidden` on `body`, which phones did not apply reliably | a fixed overhang instead of a viewport-relative one, and `overflow-x: clip` on both `html` and `body`. `hidden` was doubly wrong: it also forces the computed `overflow-y` to `auto`, making the body a scroll container and breaking the sticky top bar |
